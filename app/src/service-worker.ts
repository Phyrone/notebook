...
const CACHE = `noteapp-cache-${version}`;
...
const OFFLINE_PAGE_PATH = '/index.html';
...
sw.addEventListener('fetch', (event) => {
	...
	if (matchUrl.origin !== location.origin) return;
	...
	async function respond() {
		...
		try {
			...
			//apparently this page is not in the cache and we now return the standard offline page
			if (!url.pathname.startsWith(ASSETS_PREfIX)) {
				const offlinePage = await cache.match(OFFLINE_PAGE_PATH);
				if (offlinePage) {
					return offlinePage;
				}
			}
